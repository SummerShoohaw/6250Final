...
namespace ConsoleApp1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            bool IsCheckmated = game.IsCheckmated(Player.White);
            bool IsWinner = game.IsWinner(Player.White);
            //Console.WriteLine("Special event? {0}", status.Event);
            Console.WriteLine("Checkmate {0}?", IsCheckmated);
            Console.WriteLine("Winner {0}?", IsWinner);
            ...
            if (!hasValidMoves)
            {
                Console.WriteLine("Black has no valid moves, you win!");
                Console.ReadKey();
                return;
            }
            ...
            Console.WriteLine("Valid moves for black:");
            foreach (var move in validMoves)
            {
                movestotal += 1;
                allValidMoves.Add(move);
                Console.WriteLine("From " + move.OriginalPosition.ToString() + " to " + move.NewPosition.ToString());
            }
            Console.WriteLine("How many valid moves does black have in total? {0}", movestotal);
            Console.WriteLine("Pick one at random (not a very smart chess engine :-)");

            // let's pick a possible move using a random number generator
            int pickone = generator.Next(movestotal);
            Move blackmoves = allValidMoves[pickone];
            bool isValidB = game.IsValidMove(blackmoves);
            if (isValidB)
            {
                Console.WriteLine("Applying move from " + blackmoves.OriginalPosition.ToString() + " to " + blackmoves.NewPosition.ToString());
                MoveType typeB = game.ApplyMove(blackmoves, true);
                Console.WriteLine("Move type: {0}", typeB);
            }
